...
def create_H_fix_fig(T_vals, H_fix_res, H, data=None, language='eng'):
    ...
    def theory_swap_at_Tm(T_vals, f_hf, f_lf, Tm, enabled=False):
        """
        Разделяет теоретические кривые по Tm и при необходимости
        меняет HF и LF местами в области T > Tm.
        """
        mask_lo = T_vals <= Tm
        mask_hi = T_vals >= Tm
        ...
        return T_lo, T_hi, hf_lo, hf_hi, lf_lo, lf_hi
    ...
    T_lo, T_hi, f_hf_lo, f_hf_hi, f_lf_lo, f_lf_hi = theory_swap_at_Tm(
        T_vals=T_vals,
        f_hf=f1,
        f_lf=f2,
        Tm=Tm,
    ...
    fig.add_trace(go.Scatter(
        x=T_lo, y=f_hf_lo, mode='lines', name='HF',
        line=dict(width=2, color=HF_COLOR)
    ))
    fig.add_trace(go.Scatter(
        x=T_lo, y=f_lf_lo, mode='lines', name='LF',
        line=dict(width=2, color=LF_COLOR)
    ))

    fig.add_trace(go.Scatter(
        x=T_hi, y=f_hf_hi, mode='lines', name='HF',
        line=dict(width=2, color=HF_COLOR)
    ))
    fig.add_trace(go.Scatter(
        x=T_hi, y=f_lf_hi, mode='lines', name='LF',
        line=dict(width=2, color=LF_COLOR)
    ))
...
def create_freq_fig(T_vals, H_vals, freq_res_grid, language='eng'):
    ...
    def theory_swap_at_Tm(T_vals, f_hf, f_lf, Tm, enabled=False):
        """
        Разделяет теоретические поверхности по Tm и при необходимости
        меняет HF и LF местами в области T > Tm.
        """
        mask_lo = T_vals <= Tm
        mask_hi = T_vals >= Tm
    ...
    f2_grid = np.asarray(f2_grid, dtype=float)
    ...
    fig.add_trace(go.Surface(
        ...
        showscale=False,
        opacity=0.25,
        name=tr['temperature_title'].format(value=f"{Tm:g}"),
        hoverinfo="skip"
    ))
