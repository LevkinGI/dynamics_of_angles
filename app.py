...
def update_graphs(H, T, material, calc_on,
                 alpha_scale, chi_scale, k_scale, m_scale):
    ...
    params_changed   = any(('alpha-scale-slider' in ti or 'chi-scale-slider' in ti
                            or 'k-scale-slider' in ti or 'm-scale-slider' in ti) for ti in triggered_inputs)
    ...
    f1_T, f2_T = compute_frequencies_H_fix(H, m_array, chi_array, K_array, gamma)
    f1_H, f2_H = compute_frequencies_T_fix(H_vals, m_val, chi_val, K_val, gamma)
    H_data = H_1000 if H==1000 and material == '1' else None
    T_data = T_293 if T==293 and material == '1' else None
    ...
    return H_fix_fig, T_fix_fig, phi_fig, theta_fig, yz_fig, phi_amp_fig, theta_amp_fig, freq_fig
